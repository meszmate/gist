...
export function LessonPlayerSummary({
  ...
  const masteryConfig = {
    strong: { label: t("lessons.strongMastery"), color: "text-green-600 dark:text-green-400", bg: "bg-green-500/10" },
    moderate: { label: t("lessons.moderateMastery"), color: "text-yellow-600 dark:text-yellow-400", bg: "bg-yellow-500/10" },
    weak: { label: t("lessons.needsReview"), color: "text-red-600 dark:text-red-400", bg: "bg-red-500/10" },
  };
    ...
    <div className="flex flex-col items-center justify-center min-h-[60vh] space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-3 gap-4 w-full max-w-md">
        <Card>
          <CardContent className="p-4 text-center">
            ...
            <div className="text-xs text-muted-foreground">{t("lessons.scoreLabel")}</div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4 text-center">
            ...
            <div className="text-xs text-muted-foreground">{t("lessons.correctLabel")}</div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4 text-center">
            ...
            <div className="text-xs text-muted-foreground">{t("lessons.timeLabel")}</div>
          </CardContent>
        </Card>
      </div>
      ...
      <Card className="w-full max-w-md">
        <CardContent className="p-4">
          <h3 className="font-medium mb-3">{t("lessons.stepBreakdown")}</h3>
              ...
              return (
                <div key={step.id} className="flex items-center gap-2 text-sm">
                  ...
                  <span className="flex-1 truncate text-muted-foreground">
                    {interactive ? (result?.isCorrect ? t("lessons.correctStep") : t("lessons.incorrectStep")) : t("lessons.viewedStep")}
                  </span>
                  {interactive && result && (
                    result.isCorrect ? (
                  ...
                  {result && !result.firstAttemptCorrect && result.isCorrect && (
                    <span className="text-xs text-muted-foreground">
                      {t("lessons.tries", { count: result.attempts })}
                    </span>
                  )}
                </div>
              );
